...
export function LdapSettingsClient({ language }: LdapSettingsClientProps) {
  ...
  return (
    <Card>
      <CardContent className="p-8">
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as "config" | "test")} className="mb-6">
        <TabsList>
          <TabsTrigger value="config">{t("LDAP Configuration", "LDAP設定")}</TabsTrigger>
          <TabsTrigger value="test">{t("Connection Test", "接続テスト")}</TabsTrigger>
        </TabsList>
      </Tabs>
      ...
      {message && (
        <Alert
          variant={message.includes(t("successfully", "成功")) ? "default" : "destructive"}
          className={`mb-6 ${
