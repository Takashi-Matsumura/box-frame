...
// スコア範囲を考慮したグレード判定
function determineGradeWithRange(
  score: number,
  scoreRange: ScoreRange,
): string {
  const { min, max } = scoreRange;
...
// スコア変換テーブルを生成（範囲表示）
function generateScoreConversionTable(
  scoreRange: ScoreRange,
): Array<{ rate: string; score: string }> {
  const { min, max } = scoreRange;
  const range = max - min;
      ...
      <CardContent className="p-4">
        ...
        <div className="text-center py-2">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            ×{weight}% ={" "}
            <span className="font-medium">{weightedScore.toFixed(2)}</span>
          </p>
        </div>
...
export default function EvaluationForm({
  ...
  const scoreRange = useMemo(() => {
    ...
    return calculateScoreRange(
      evaluation.processCategories,
      evaluation.growthCategories,
    );
  }, [evaluation]);
  ...
  const isAllComplete =
    isScore1Complete && isScore2Complete && isScore3Complete;
  ...
  const buildEmployeeInfo = () => {
    ...
    const lines = [];
    lines.push(
      `${language === "ja" ? "氏名" : "Name"}: ${getEmployeeName(emp)}`,
    );
    if (emp.position)
      lines.push(`${language === "ja" ? "役職" : "Position"}: ${emp.position}`);
    if (emp.qualificationGrade)
      lines.push(
        `${language === "ja" ? "等級" : "Grade"}: ${emp.qualificationGrade}`,
      );
    if (emp.department)
      lines.push(
        `${language === "ja" ? "所属" : "Department"}: ${emp.department.name}`,
      );
    return lines.join("\n");
  };
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-muted-foreground">
                    ...
                    {evaluation.employee.course &&
                      ` / ${evaluation.employee.course.name}`}
                  </p>
                </div>
                      ...
                      {evaluation.employee.birthDate && (
                        <div className="text-center">
                          ...
                          <p className="font-medium">
                            {calculateAge(evaluation.employee.birthDate)}
                            {language === "ja" ? "歳" : ""}
                          </p>
                        </div>
                      )}
                      ...
                      <div className="grid grid-cols-5 gap-2">
                        {scoreConversionTable.map((item, index) => {
                          const achievementRate =
                            evaluation.organizationGoal?.achievementRate ?? 0;
                          const isActive =
                            index === 0
                              ? achievementRate >= 160
                              : index === 1
                          ...
                          return (
                            ...
                            >
                              ...
                              <p
                                className={`font-bold ${isActive ? "text-blue-600 dark:text-blue-400" : ""}`}
                              >
                                → {item.score}
                              </p>
                            </div>
            ...
            <Card className="animate-in slide-in-from-top-2">
              ...
              <CardContent>
                <ProcessEvaluationSupport
                  ...
                  initialProcesses={
                    processProjects.length > 0 ? processProjects : undefined
                  }
                />
              </CardContent>
            </Card>
                  ...
                  <div className="flex items-start gap-2 p-3 rounded-lg bg-yellow-500/10 border border-yellow-500/30">
                    ...
                    <div className="text-sm">
                      <p className="font-medium text-yellow-600 dark:text-yellow-400">
                        {language === "ja"
                          ? "未入力の評価項目があります"
                          : "Some evaluation items are incomplete"}
                      </p>
                  ...
                  <table className="w-full">
                    ...
                    <tbody className="text-sm">
                      <tr className="border-b">
                        ...
                        <td className="text-center font-mono py-3 px-4">
                          {score1.toFixed(1)}
                        </td>
                        <td className="text-center py-3 px-4">
                          {evaluation.weights.resultsWeight}%
                        </td>
                        <td className="text-right font-mono font-bold py-3 px-4">
                          {weightedScore1.toFixed(2)}
                        </td>
                        <td className="text-center py-3 px-4">
                          {isScore1Complete ? (
                ...
                <div className="flex items-center justify-between p-4 rounded-lg bg-muted/50">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                      {finalScore >= 4.5 &&
                        (language === "ja" ? "卓越" : "Excellent")}
                      {finalScore >= 3.5 &&
                        finalScore < 4.5 &&
                        (language === "ja" ? "優秀" : "Good")}
                      {finalScore >= 2.5 &&
                        finalScore < 3.5 &&
                        (language === "ja" ? "標準" : "Standard")}
                      {finalScore >= 1.5 &&
                        finalScore < 2.5 &&
                        (language === "ja"
                          ? "改善が必要"
