...
2. **成長評価マスタ** - 各カテゴリのT1〜T4スコア（係数は適用しない）

> **注意**: 成長評価の係数は個別のスコア計算時にのみ使用され、スコア範囲の決定には影響しません。これにより、係数が大きいカテゴリがあっても、スコア範囲が過度に広がることを防ぎます。
...
※ 成長評価の係数（×1.0、×1.3など）はスコア範囲の計算には含めません。
...
プロセス評価・成長評価のマスタデータを変更した場合、スコア範囲が変わるため、既存の結果評価スコアを再計算する必要があります。
...
これにより、全社員の結果評価スコア（score1）が新しいスコア範囲で再計算されます。

### 再計算の対象

- 評価期間に紐づく全評価レコード
- 評価対象外（EvaluationExclusion）の社員はスキップ
- 達成率が設定されていない組織の社員もスキップ

### API

管理者権限で以下のAPIを直接呼び出すことも可能です：

```bash
POST /api/evaluation/criteria1/sync-scores
